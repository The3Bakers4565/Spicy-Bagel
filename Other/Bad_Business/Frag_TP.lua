...
local PlayerTable=getupvalue(require(game:GetService("ReplicatedStorage").TS).Characters.GetCharacter,1)--so game dosent lag alot
        ...
        if v.Name~=ClientTeam then
            for _,c in pairs(v.Players:GetChildren())do
                if PlayerTable[c.Value]then
                    if PlayerTable[c.Value].Parent==game:GetService("Workspace").Characters then
...
game:GetService("Workspace").Throwables.ChildAdded:Connect(function(x)
    ...
    if target then 
        repeat  
            ...
            game:GetService("RunService").RenderStepped:Wait()--renderstepped:wait > wait
        until x==nil or not x:IsDescendantOf(game:GetService("Workspace"))--stop moving nade when deleted
    end
end)
game:GetService("Workspace").ChildAdded:Connect(function(x)--fix for new rounds
    if x.Name=="Throwables"then
        x.ChildAdded:Connect(function(x)
            ...
            if target then 
                ...
                until x==nil or not x:IsDescendantOf(game:GetService("Workspace"))
            end
        end)
    end
end)
